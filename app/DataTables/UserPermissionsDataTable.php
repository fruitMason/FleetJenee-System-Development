...
class UserPermissionsDataTable extends DataTable
{
    ...
    public function dataTable($query): DataTableAbstract
    {
        return datatables()->eloquent($query)
            ...
            ->addColumn('name', function($row) {
                return User::query()->find($this->user_id)->full_name();
            })
            ...
            ->addColumn('action', function ($row) {
                return '<div class="btn-group" role="group" aria-label="Action buttons">' .
                    ...
                    'onclick="resolve('.$row->id.')"><i class="fas fa-check text-white" aria-hidden="true"></i></a>' .
                    '<a class="btn btn-xs btn-danger" href="#"' .
                    'onclick="delete_notify('.$row->id.')"><i class="fas fa-trash text-white" aria-hidden="true"></i></a>' .
                    '</div>';
            })->rawColumns(['checkbox', 'permissions', 'status', 'action', 'respond']);
    }
    ...
    {
        $query = Role::findById($this->role_id)->with(['permissions'])->whereHas('users', function ($q){
    ...
    protected function getColumns(): array
    {
        return [
            ...
            Column::make('permissions')->addClass('text-center no-border'),
