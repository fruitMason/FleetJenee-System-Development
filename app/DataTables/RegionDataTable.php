...
class RegionDataTable extends DataTable
{
            ...
            ->addColumn('action', function ($row) {
                if ($row->id == 1) {
                    return
                        ...
                        'onclick="editNotify(' . $row->id . ')"><i class="fa fa-edit text-white" aria-hidden="true"></i></a></div>';
                }
    ...
    {
        $query = Region::query()->with(['sector'])
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('sector')->addClass('text-center no-border'),
    ...
    {
        return 'RegionDataTable_' . date('YmdHis');
    }
}
